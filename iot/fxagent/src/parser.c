...
int cloudc_parse_http_header(char *recvbuf)
{
    ...
    char tmp1[32] = {0};
    char returnCode[8] = {0};
    sscanf(recvbuff,"%s %s %s",tmp1, returnCode, tmp);
    cloudc_debug("%s[%d]: returnCode = %s", __func__, __LINE__, returnCode);
...
int cloudc_parse_http_body(char *json_buf)
{
    ...
    cJSON *json, *json_type, *json_serial, *json_status, *json_package, *json_update, *json_url, *json_user_id, *json_device_id, *json_config;
            ...
            switch(recvdata.rpc_flag)
            {
                ...
                case eAlljoynSetOperation:
                    ...
                    else
                    {
                        ...
                        cJSON *json_keyNode = cJSON_GetObjectItem(json_config, "devData");
                        if(json_keyNode == NULL)
                        {
                            cloudc_debug("%s[%d]: cannot find devData key in config\n", __func__, __LINE__);
                        }
                        else
                        {
                            ...
                            if(s)
                            {
                                ...
                                cloudc_debug("%s[%d]: recvdata.devData = %s", __func__, __LINE__, recvdata.devData);
                                free(s);
                            }
                        }
                    }
